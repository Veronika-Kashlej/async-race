import { driveEndine } from '../../../db/engine/driveEngine';
import { CarResponse } from '../../../db/garage/postCar';
import { removeCar } from '../../../db/garage/removeCar';
import { startEndine } from '../../../db/engine/startEngine';
...
import GarageRaceView from './race-view';
import './race.css';
import './modal.css';
import { postWinner } from '../../../db/winner/postWinner';
...
import { getWinner } from '../../../db/winner/getWinner';
import { updateWinner } from '../../../db/winner/updateWinner';
export const CAR_SVG = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
...
export default class CarLineView {
  static selectedCarId: number;
  static selectedCarElement: ElementCreator;
  static carName: ElementCreator;
  elementCreator: ElementCreator;
  carElement: HTMLElement | null;
  carData: CarResponse;
  private currentPosition: number;
  private animationFrameId: number | null;
  startBtn: ElementCreator | null;
  stopBtn: ElementCreator | null;
  ...
  private configureHeader() {
    const liHeader = new ElementCreator({ tag: 'div', classNames: ['div'] });
    this.elementCreator.addInnerElement(liHeader);
    const selectBtn = new ElementCreator({
      tag: 'button',
      classNames: ['button'],
      textContent: 'Select',
      callback: () => {
        const name = this.carData.name;
        const color = this.carData.color;
        CarLineView.selectedCarElement = this.elementCreator;
        CarLineView.selectedCarId = this.getSelectedCar().id;
        if (name) {
    ...
    liHeader.addInnerElement(selectBtn);

    const removeBtn = new ElementCreator({
      tag: 'button',
      classNames: ['button'],
      textContent: 'Remove',
      callback: () => {
        this.elementCreator.getElement()?.remove();
        removeCar(this.carData.id);
        removeWinner(this.carData.id);
        GarageRaceView.updateCarList();
      },
    });
    liHeader.addInnerElement(removeBtn);

    CarLineView.carName = new ElementCreator({
      tag: 'span',
      classNames: ['span'],
      textContent: this.carData.name,
    });
    liHeader.addInnerElement(CarLineView.carName);
  }
  private configureTrack() {
    const liTrack = new ElementCreator({ tag: 'div', classNames: ['div'] });
    this.elementCreator.addInnerElement(liTrack);

    this.startBtn = new ElementCreator({
      tag: 'button',
      classNames: ['button', 'start'],
      textContent: 'Start',
      callback: () => {
        if (this.startBtn && this.stopBtn) {
          const startElement = this.startBtn.getElement();
          const stopElement = this.stopBtn.getElement();
    ...
    liTrack.addInnerElement(this.startBtn);

    this.stopBtn = new ElementCreator({
      tag: 'button',
      ...
      callback: () => {
        if (this.startBtn && this.stopBtn) {
          ...
          const stopElement = this.stopBtn.getElement();
          if (startElement && stopElement) {
            stopElement.setAttribute('disabled', 'true');
    ...
    this.stopBtn.getElement()?.setAttribute('disabled', 'true');
    liTrack.addInnerElement(this.stopBtn);

    const carImg = new ElementCreator({
      tag: 'div',
      classNames: ['car-icon'],
    });
    this.carElement = carImg.getElement();
    if (this.carElement) {
      this.carElement.innerHTML = CAR_SVG.replace(/fill:#272525;/g, `fill:${this.carData.color};`);
    }
    liTrack.addInnerElement(carImg);

    const flagImg = new ElementCreator({ tag: 'img', classNames: ['img', 'flag'] });
    const flagImgElement = flagImg.getElement();
    if (flagImgElement && flagImgElement instanceof HTMLImageElement) {
      flagImgElement.src = 'assets/img/flag.png';
      flagImgElement.alt = 'finish';
    }
    liTrack.addInnerElement(flagImg);
  }
  async startAnimation() {
    const response = await startEndine(this.carData.id);
    const trackWidth = this.findTrackWidth();
    const duration = response.distance / response.velocity;
    let startTime: number | null = null;
    const raceStartTime = performance.now();
    const drivePromise = driveEndine(this.carData.id).catch((error) => {
    ...
    const animate = (timestamp: number) => {
      ...
      if (carElement) {
        if (progress < 1) {
          this.currentPosition = trackWidth * progress;
          carElement.style.transform = `translateX(${this.currentPosition}px)`;
          this.animationFrameId = requestAnimationFrame(animate);
        } else {
          if (!GarageFormView.winnerCar && !this.isBroken && GarageFormView.isRace) {
            GarageFormView.winnerCar = this.carData;
            const finishTime = +((performance.now() - raceStartTime) / 1000).toFixed(2);
            this.updateWinner(finishTime);
            this.showWinner(finishTime);
          }
  ...
  private async updateWinner(time: number) {
    try {
      const existingWinner = await getWinner(this.carData.id);
      const minTime = Math.min(existingWinner.time, time);
      await updateWinner({ id: this.carData.id, wins: existingWinner.wins + 1, time: minTime });
    } catch {
      await postWinner({ id: this.carData.id, wins: 1, time: time });
    }
  }
  showWinner(time: number) {
    const winnerMessage = new ElementCreator({ tag: 'div', classNames: ['winner-message'] });
    const winnerContainer = new ElementCreator({ tag: 'div', classNames: ['winner-popup'] });
    const title = new ElementCreator({ tag: 'h3', classNames: ['title'], textContent: 'Winner!' });
    const winnerName = new ElementCreator({
      tag: 'p',
      classNames: ['text'],
      textContent: `Car: ${this.carData.name}`,
    });
    const winnerTime = new ElementCreator({
      tag: 'p',
      classNames: ['text'],
      textContent: `Time: ${time}s`,
    });
    winnerContainer.addInnerElement(title);
    winnerContainer.addInnerElement(winnerName);
    winnerContainer.addInnerElement(winnerTime);
    winnerMessage.addInnerElement(winnerContainer);
    const modalElement = winnerMessage.getElement();
    if (modalElement) {
      this.elementCreator.addInnerElement(modalElement);
      setTimeout(() => {
        modalElement.remove();
      }, 3000);
    }
  }
  private findTrackWidth() {
    const startBtn = document.querySelector('.button.start');
    const stopBtn = document.querySelector('.button.stop');
    const flagImg = document.querySelector('.flag');
    const containerWidth = this.elementCreator.getElement()?.clientWidth;
    let totalWidthElements = 0;
    let totalWidth = 0;
    const elements = [startBtn, stopBtn, flagImg, this.carElement];
    ...
    if (containerWidth) {
      totalWidth = containerWidth - totalWidthElements - 40;
    }
    return totalWidth;
  }
  stopAnimation() {
  ...
  displayName(name: string) {
    GarageFormView.inputNameUpdate.setValue(name);
  }
  displayColor(color: string) {
    GarageFormView.inputColorUpdate.setValue(color);
  }
  getSelectedCar() {
    return this.carData;
  }
